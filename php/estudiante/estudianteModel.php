<?php
    class estudianteModel{
        ...
        public function insertar($nombre,$apellido,$idCurso){
            $sql = 'INSERT INTO estudiantes VALUE (0,:nombre,:apellido,:idCurso)';
        ...
        public function listar() {
            $sql = 'SELECT e.idEstudiante,e.nombre,e.apellido,e.idCurso,c.nombre as curso FROM 
            estudiantes e JOIN cursos c ON e.idCurso = c.idCurso ORDER BY idEstudiante DESC';
            $statement = $this->PDO->prepare($sql);
        ...
        public function buscar($idEstudiante){
            $sql = 'SELECT e.idEstudiante,e.nombre,e.apellido,e.idCurso,c.nombre as curso FROM 
            estudiantes e JOIN cursos c ON e.idCurso = c.idCurso
